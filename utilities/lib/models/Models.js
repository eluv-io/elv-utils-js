...
const CheckedAbsentPropName = curry((errorMessage, object, propertyName) => !Object.keys(object).includes(propertyName)
  ? Ok(propertyName)
  : Err(Error(`"${propertyName}" ${errorMessage || ' already exists'}`))
)

const CheckedPresentPropName = curry((errorMessage, object, propertyName) => Object.keys(object).includes(propertyName)
  ? Ok(propertyName)
