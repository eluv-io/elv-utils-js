...
const New = (context) => {
  ...
  // save exit code into JSON output
  ...
  const outputJSON = () => {
    if (json) {
      const lines = JSON.stringify(redact(output), null, 2).split('\n')
      // eslint-disable-next-line no-console
      if (!silent) lines.forEach(x => console.log(x))
    }
  }

  // save resolvedConf into JSON output
  const resolvedConf = (obj) => output.resolvedConf = obj
