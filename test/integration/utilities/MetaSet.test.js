// Integration test for https://docs.eluv.io/docs/guides/media-ingest/elv-utils-js/utilities/#metasetjs
'use strict'
...
describe('MetaGet.js', () => {
  it('should set metadata successfully when there is an array along the traversal path', async function () {
    const draftInfo = await ITH.createDraft({
      name: TH.timestampFilename(__filename) + ' array traversal tests',
      libraryId: testConfig.mezLib,
      metadata: JSON.stringify(SAMPLE_METADATA)
    })

  })
  it('should throw an error when there is an array along the traversal path and index is incorrect', async function () {

  })
  it('should set metadata in a draft successfully, from a finalized object successfully, and error if draft not found', async function () {
    this.timeout(60000)
