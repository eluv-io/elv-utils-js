...
const ObjectDelete = TH.requireUtility('ObjectDelete')
...
const deleteObject = async ({objectId}) => {
  ...
  const result = await new ObjectDelete({argList}).run()
  if (result.exitCode !== 0) {throwError`deleteObject failed: ${result.errors}`}
  return result.data
}
